...
function formatUsingTxtpbfmt(document: vscode.TextDocument): vscode.TextEdit[] {
  const txtpbfmtOption =
    vscode.workspace.getConfiguration("textproto.txtpbfmt");
  const txtpbfmtPath = txtpbfmtOption.get("executable", "txtpbfmt");

  if (
    !(
      isCommandAvailable(txtpbfmtPath) ||
      isExecutableFileAvailable(txtpbfmtPath)
    )
  ) {
    vscode.window.showErrorMessage(
      `txtpbfmt (path: ${txtpbfmtPath}) executable not found.\n` +
  ...
  let formatResult = cp.execFileSync(txtpbfmtPath, args, {
